...
```bash
# dist 폴더 내용을 VM으로 전송
scp -r dist/* user@VM_EXTERNAL_IP:/var/www/html/
```
...
### 8.3 Django Static 파일 복사

```bash
# Docker 볼륨에서 static 파일 복사
...
sudo chown -R www-data:www-data /var/www/static /var/www/media
```

---

## Step 9: Cloudflare 설정
